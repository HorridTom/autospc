...
create_SPC_auto_limits_table <- function(data, 
  ...
  #manually places breakpoints
  if(!is.null(manual_override_breakpoints)){
    ...
    for(breakpoint in manual_override_breakpoints){
      limits_table <- form_calculation_and_display_limits(limits_table, periodMin, counter = breakpoint, chartType, maxNoOfExclusions)
    }
