          ...
          if (is.na(rule2_break_position) |
            ...
            if (!enough_data_for_new_period(
              ...
              opposite_rule_break <- identify_opposite_break(
                ...
                overhanging_reversions = chart$overhanging_reversions
              )[[1]]
              ...
              period_end <- min(
                counter + chart$period_min - 1L,
                nrow(limits_table)
              )
              prevailing_row <- if (counter > 1L) {
                as.integer(counter) - 1L
              } else {
                NA_integer_
              }
