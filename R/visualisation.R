...
create_spc_plot <- function(plot_data,
  ...
  plot_unformatted <- ggplot2::ggplot(
    long_table %>%
  ...
  spc_plot <- format_spc_plot(
    ...
    ggplot2::labs(
      x = visualisation_params$override_x_title,
...
format_spc_plot <- function(plot_unformatted,
  ...
  first_display_period <- plot_periods[grep(
    "display",
    plot_periods
  )[1]]
  ...
  if (any(x_class == "Date")) {
    if (is.null(x_break)) {
      spc_plot <- spc_plot +
        ggplot2::scale_x_date(
          labels = scales::date_format(x_date_format),
          breaks = scales::breaks_pretty(),
          limits = c(
            as.Date(start_x),
            as.Date(end_x)
    ...
    if (is.null(x_break)) {
      ...
      if (any(class(x_break) != "difftime")) {
        rlang::abort(paste(
          "Please specify x_break as a difftime object when",
          "x is POSIXct."
        ))
      }
      spc_plot <- spc_plot +
        ggplot2::scale_x_datetime(
          breaks = seq(start_x, end_x, x_break),
          limits = c(start_x, end_x)
        )
    }
  } else {
