...
sets out some useful terminology, and describes the algorithm.
...
![](stable-shift-algorithm_files/figure-html/2.0-1.png)
...
the threshold for a rule break (`runRuleLength`), set to $8$ by default
in
[`plot_auto_SPC()`](https://horridtom.github.io/autospc/reference/plot_auto_SPC.md).

Runs that are subsets of longer runs count here, so for example with the
default `runRuleLength = 8`, a run of length 10 actually comprises three
rule-breaking runs, commencing at the first, second and third points in
the length 10 run. In this example, the first rule-breaking run has
length 10, the second 9, and the third 8. The run that commences on the
...
re-establish limits. Such a run is referred to as a *triggering rule
break*.
...
original limits. If the rule break only reaches the `runRuleLength`
threshold after the end of the candidate calculation period, it is
referred to as an *overhanging reversion*.

### Minimum period length

The SSA requires specification of a minimum number of data points to be
used for calculation of control limits, $n_{min}$. Whilst those using
...
In
[`plot_auto_SPC()`](https://horridtom.github.io/autospc/reference/plot_auto_SPC.md),
$n_{min}$ is specified by the `periodMin` argument, defaulting to 21.
...
2.  Check there is sufficient data to form at least one set of limits.
    If a time series has fewer points than $n_{min}$, then according to
    the SSA no control limits can be established. If a time series has
    length $n$ with $n_{min} \leq n < 2n_{min}$, then there are
    sufficient data points to form one period. The SSA establishes
    limits calculated from the first $n_{min}$ points, with limits
...
time series to be analysed is $n \geq 2n_{min}$. After the first set of
limits are established, the counter is set to the first point after the
end of this first calculation period.

3.  Main algorithm loop begins

4.  If there are fewer than $n_{min}$ data points remaining from the
...
6.  Again check there is sufficient data remaining from the counter. If
    ...
    $n_{min}$ data points beginning at the counter.

7.  Decide whether to accept or reject the candidate limits:

    1.  If there at least one opposing rule break commencing within the
        candidate calculation period, then the candidate limits are
