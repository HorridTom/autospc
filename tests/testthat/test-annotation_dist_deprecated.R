annotation_dist_data <- data.frame(
  x = 1:30,
  y = c(
    49, 50, 50, 50, 48, 49, 50, 49, 50, 47,
...
test_that("override_annotation_dist throws a deprecation error", {
  expect_error(
    run_annotation_dist(override_annotation_dist = 10),
    "deprecated"
  )
})
...
test_that("override_annotation_dist_P throws a deprecation error", {
  expect_error(
    run_annotation_dist(override_annotation_dist_P = 10),
    "deprecated"
  )
})
