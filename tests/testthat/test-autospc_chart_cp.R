...
test_that("validate_autospc_chart_cp rejects a bare autospc_chart object", {
  expect_error(
    validate_autospc_chart_cp(new_autospc_chart(assemble_chart_list(
...
test_that("validate_autospc_chart_cp rejects an autospc_chart_c object", {
  expect_error(
    validate_autospc_chart_cp(autospc_chart_c(
      data = test_data,
      x = "x",
      y = "y"
    )),
...
test_that("calculate_limits takes mr_screen_max_loops from the chart", {
  ...
  expect_false(
    identical(
      calculate_limits(test_chart_cp(mr_screen_max_loops = 0L),
...
test_that("the C' chart y axis leaves headroom above the upper limit", {
  expect_identical(
    y_axis_range(test_chart_cp(), data = limits_data),
