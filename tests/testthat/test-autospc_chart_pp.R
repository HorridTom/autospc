...
pp_sub_level_data <- data.frame(
  x = rep(1:3, each = 2),
  y = c(1, 2, 3, 4, 5, 6),
  n = rep(10L, 6)
)

pp_binary_data <- data.frame(
  x = rep(1:5, each = 4),
  y = rep(c(TRUE, FALSE, TRUE, TRUE), 5)
)

# one observation per subgroup, so every proportion is 0% or 100%
pp_binary_degenerate_data <- data.frame(
  x = 1:5,
  y = c(TRUE, FALSE, TRUE, TRUE, FALSE)
)

# a calculation period as the algorithm builds it: series holds percentages and
# y holds the counts
...
pp_screening_data <- data.frame(
  ...
  y = pp_screening_numerator,
  n = rep(20, length(pp_screening_numerator)),
  series = pp_screening_numerator * 100 / 20
)
...
test_that("autospc_chart_pp returns an object of the expected class", {
  expect_identical(
    class(chart_pp(pp_pre_agg_data)),
...
test_that("n is required and is appended after the common elements", {
  expect_error(
    autospc_chart_pp(data = pp_pre_agg_data, x = "x", y = "y"),
...
test_that("a missing n is caught on the constructor path", {
  ...
  expect_error(
    validate_autospc_chart_pp(no_n),
...
test_that("validate_autospc_chart_pp rejects an autospc_chart_p object", {
  expect_error(
    validate_autospc_chart_pp(autospc_chart_p(
      data = pp_pre_agg_data,
      x = "x",
      y = "y",
      n = "n"
...
test_that("aggregate_data returns the same columns by either route", {
  expect_identical(
    names(aggregate_data(chart_pp(pp_pre_agg_data))$data),
...
test_that("calculate_limits takes mr_screen_max_loops from the chart", {
  ...
  unscreened <- calculate_limits(
    chart_pp(pp_pre_agg_data,
      mr_screen_max_loops = 0L
    ),
    period = pp_screening_data,
    exclusion_points = NULL
  )
  screened <- calculate_limits(
    chart_pp(pp_pre_agg_data,
      mr_screen_max_loops = 1L
    ),
    period = pp_screening_data,
...
test_that("limits_table_columns keeps y and n", {
  expect_identical(
    limits_table_columns(chart_pp(pp_pre_agg_data)),
    c("y", "n")
  )
})
...
test_that("P' chart labels are rounded to one decimal place", {
  expect_identical(
    label_accuracy(chart_pp(pp_pre_agg_data), ylimhigh = 110),
    0.1
  )
})
...
test_that("a P prime chart label is a percentage", {
  expect_identical(
    centre_line_label(chart_pp(pp_pre_agg_data),
      cl = 43.28,
      ylimhigh = 110
    ),
    "43.3%"
...
test_that("the limits table carries the period's sd estimate", {
  ...
  expect_length(unique(result$sd_estimate), 1L)
