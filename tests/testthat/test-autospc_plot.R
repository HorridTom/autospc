...
test_that("the plot carries the charts it was drawn from", {
  ...
  expect_identical(
    autospc_plot_charts(test_plot(charts = list(chart))),
    list(chart)
  )
})
...
test_that("the plot carries the visualisation parameters", {
  settings <- list(show_limits = FALSE, point_size = 4)

  drawn_with <- test_plot(visualisation_params = settings)

  expect_identical(autospc_plot_visualisation_params(drawn_with), settings)
})
...
test_that("a single visualisation parameter can be read by name", {
  plot <- test_plot(visualisation_params = list(
    show_limits = FALSE,
    point_size = 4
  ))
...
test_that("validate rejects an object that is not an autospc_plot", {
  expect_error(
    validate_autospc_plot(built_plot()),
...
test_that("validate rejects missing elements", {
  ...
  expect_error(
    validate_autospc_plot(no_charts),
...
test_that("validate rejects charts that are not autospc_chart objects", {
  expect_error(
    test_plot(charts = list(plot_data)),
...
test_that("validate rejects a half that is not a named list", {
  expect_error(
    test_plot(visualisation_params = c(show_limits = TRUE)),
    "presentation\\$visualisation_params must be a list"
  )

  expect_error(
    test_plot(visualisation_params = list(TRUE)),
    "presentation\\$visualisation_params must be named"
  )

  expect_error(
    test_plot(axis_extents = list(110)),
...
test_that("validate rejects a presentation missing a half", {
  ...
  expect_error(
    validate_autospc_plot(half_only),
