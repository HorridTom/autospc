...
test_that("", {
  ...
  expect_equal(
    length(levels(faceted_build$data[[1]]$PANEL)),
    4L
  )
})
...
test_that("it records the axis extents it was drawn with", {
  ...
  expect_setequal(
    names(autospc_plot_axis_extents(plot)),
  ...
  expect_identical(
    autospc_plot_axis_extents(plot, value = "start_x"),
    min(drawn$x)
  )

  expect_identical(
    autospc_plot_axis_extents(plot, value = "x_max"),
    max(drawn$x)
  )

  expect_identical(
    autospc_plot_axis_extents(plot, value = "end_x"),
    max(drawn$x)
  )
})
...
test_that("a faceted chart has no x axis label unless one is given", {
  ...
  expect_identical(
    faceted_plot(override_x_title = "Month")$labels$x,
    "Month"
  )
})
