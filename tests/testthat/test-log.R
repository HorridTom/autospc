...
test_data <- data.frame(
  x = 1L:50L,
  y = c(
    50L, 48L, 49L, 50L, 52L, 52L, 48L, 53L, 51L, 51L,
  ...
  n = c(
    98L, 104L, 94L, 104L, 102L, 103L, 102L, 100L,
...
correct_log <- structure(
  list(
    x = c(1L, 22L, 36L),
    log = c(
      "0100;0200", "0300;040136",
      "050010;0610"
    )
  ),
  row.names = c(NA, -3L),
...
correct_log_table <- structure(
  list(
    ...
    interpretation = c(
      ...
      paste0(
        "Sufficient data to proceed. Moving counter ",
        "to the next shift rule break, commencing at ",
        "point 36."
      ),
      paste0(
        "There is a shift rule break commencing here,",
  ...
  row.names = c(NA, -6L),
  groups = structure(
    list(.rows = structure(list(1L, 2L, 3L, 4L, 5L, 6L),
      ptype = integer(0),
      class = c(
        "vctrs_list_of",
        "vctrs_vctr",
...
test_that("specific log entries are interpreted correctly", {
  ...
  expect_equal(
    interpret_060011,
    paste(
      "Sufficient data to proceed. Forming candidate limits.",
  ...
  expect_equal(
    interpret_0710,
    paste(
      "Candidate limits rejected, prevailing limits",
      "retained."
    )
  )
})
...
test_that("the log covers the other chart types", {
  ...
  ed <- data.frame(
    x = ed_attendances_monthly$month_start,
...
test_that("an entry past the end of the table is held at the last row", {
  ...
  exact <- data.frame(
    x = 1:21,
    y = c(
      10, 12, 11, 13, 9, 10, 12, 11, 13, 9, 10,
  ...
  analysed <- suppressWarnings(establish_limits(prepare_data(
    autospc_chart(
      chart_type = "C\'", data = exact, x = "x", y = "y",
      period_min = 21L
    )
  )))
