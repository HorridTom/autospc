...
print_data <- data.frame(
  x = 1:60,
  y = c(
    rep(c(10, 12, 11, 13, 9, 14, 10, 11, 12, 13), 3),
...
test_that("print omits the exclusions line when nothing was excluded", {
  ...
  expect_no_match(
    paste(capture.output(print(chart)), collapse = " "),
    "excluded"
  )
})
...
test_that("print reports why the run stopped", {
  ...
  expect_output(
    print(chart),
    paste0(
      "Stopped at row ", chart$history$stopped$counter, ": ",
...
test_that("a long analysis is truncated rather than filling the console", {
  many <- data.frame(
    period_type = rep("calculation", 30),
...
test_that("display periods are not listed as calculation periods", {
  table <- data.frame(
    period_type = c("calculation", "calculation", "display"),
    plot_period = c(
