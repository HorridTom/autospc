...
test_that("data_original keeps the class the caller passed", {
  from_tibble <- autospc_chart(
    chart_type = "C", data = return_class_tibble,
    x = "x", y = "y"
  )

  expect_s3_class(from_tibble$data_original, "tbl_df")

  from_data_frame <- autospc_chart(
    chart_type = "C", data = return_class_data,
    x = "x", y = "y"
  )
