...
test_that("Determines correctly whether there are enough data point to form a new period",{
  ...
  result1 <- enough_data_for_new_period(data = test_data, periodMin = 21,
                                        counter = 1, chartType = "C")
  
  #case when counter is much less than nrow(data)
  result2 <- enough_data_for_new_period(data = test_data, periodMin = 21,
                                        counter = 54, chartType = "C")
  
  #case when counter is close to end 
  result3 <- enough_data_for_new_period(data = test_data, periodMin = 21,
                                        counter = 140, chartType = "C")
