...
example, in Figure \@ref(fig:example-1), there is one calculation period,
covering days 1 to 21 inclusive, extended into its display period covering
day 22 onwards.

### Rule-breaking run
A *rule-breaking run* is a run whose length is greater than or equal to the
threshold for a shift rule break (`shift_rule_threshold`), set to \(8\) by
default in `autospc()`. In Figure \@ref(fig:example-1), there is a
rule-breaking run of length 10 starting on day 22. By default, rule-breaking
runs are highlighted in blue by `autospc`.
...
Figure \@ref(fig:example-1) actually comprises three rule-breaking runs,
commencing at
...
In Figure \@ref(fig:example-1) the highlighted rule-breaking run is a
triggering rule break.
...
In Figure \@ref(fig:example-2) we imagine rolling time forward, so that we
have more data to add to the chart in Figure \@ref(fig:example-1). Figure
...
If an opposing rule break only reaches the `shift_rule_threshold` threshold
after the end of the candidate calculation period, it is referred to as an
...
for calculation of control limits, \(n_{min}\). Whilst those using SPC in
practice may not often make such a minimum explicit, in a way it is always
there implicitly - nobody would compute control limits from two data points
would they? In fact, various authors offer guidance on what such a minimum
should be, with values ranging from 17 to 25 points.
...
7. Decide whether to accept or reject the candidate limits:
    ...
    i. If there is at least one opposing rule break commencing within the
    candidate calculation period, then the candidate limits are rejected, the
    counter is set to the next rule-breaking run (after the triggering rule
    break) against the prevailing limits, and the algorithm continues from
    (3).
...
```{r 5.1, fig.width=5, fig.height=7, out.width="75%"}
...
following columns:
 ...
 single chart, `"X"` or `"MR"` for the two halves of an XmR pair, and the
 stage for a faceted chart
 - `counter`        : the counter value that the log entry refers to. This is
 the row of the series the algorithm analysed, which is the data passed to the
 `data` argument of `autospc()` after it has been aggregated over `x` and put
 in `x` order
