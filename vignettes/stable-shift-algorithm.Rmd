...
```{r 2.1, fig.width=7, fig.height=4.3}
plot_auto_SPC(
  ...
  x_break = 91.25,
  x_date_format = "%Y-%b",
  x_pad_end = as.Date("2018-04-01"),
  upper_annotation_sf = 0.825
 )
```
```{r 2.2, fig.width=7, fig.height=4.3}
plot_auto_SPC(
  ed_attendances_monthly %>%
    filter(row_number() <= 24),
  x = Month_Start, 
...
```{r 2.3, fig.width=7, fig.height=4.3}
plot_auto_SPC(
  ed_attendances_monthly %>%
...
grViz(autospc:::get_algorithm_flow_chart_string())
```
...
```{r 6.2}
log_exp_tab <- autospc:::get_log_explanation_table()
```
